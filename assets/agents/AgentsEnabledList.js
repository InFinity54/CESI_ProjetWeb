...
class AgentsEnabledList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            error: null,
    ...
    render() {
        $("#agentsenabledlistloading").remove();
        ...
        if (this.state.agents.filter(agent => agent.isActivated).length > 0) {
            return this.state.agents.map((item, index) => {
                if (item.isActivated) {
                    ...
                    const disableLink = Routing.generate('agents_disable', { id: item.id }, true);
                    ...
                    }, "Actif")), /*#__PURE__*/React.createElement("td", {
                        className: "text-center"
...
const domContainer = document.querySelector('#agentsenabledlist');
ReactDOM.render(e(AgentsEnabledList), domContainer);